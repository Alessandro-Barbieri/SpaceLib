	...
	for(t=0;;t+=dt)                /* for each instant of time */
	{
		...
		for (i=1;i<=nlink;i++) /* for each link */
		{
			...
			dhtom(jtype[i],theta[i],d[i],b[i],a[i],alpha[i],q,
				  mrel[i]);/* build relative velocity and
					 acceleration matrix in local frame (4) */
			velacctoWH(jtype[i],qp,qpp,W[i],H[i]);
			molt4(T[i-1],mrel[i],T[i]);   /* evaluate absolute
							  position matrix (5) */
			trasf_mami(W[i],T[i-1],W0[i]);/* transform relative
						   velocity matrix from local
						   frame to base frame  (6) */
			trasf_mami(H[i],T[i-1],H0[i]);/* transform relative
						acceleration matrix from local
						frame  to  base frame  (7) */
			sum4(WA[i-1],W0[i],WA[i]);    /* evaluate absolute
							  velocity matrix  (8) */
		...
		actom(fx,fy,fz,cx,cy,cz,EXT);           /* build external
							action matrix */
		trasf_mamt4(EXT,T[nlink],ACT0[nlink+1]);/* transforms
							external actions from
							local to base frame */
		for(i=nlink;i>0;i--)                    /* for each link */
		{
			trasf_mamt4(J[i],T[i],J0[i]);   /* transform inertia
							matrix from local to
							base frame  (11) */
			rmolt4(HA[i],-1.,TMP);          /* change sign to
							find inertia action */
			sum4(TMP,Hg,Ht);                /* evaluate total
							acceleration matrix */
			skew4(Ht,J0[i],FI[i]);          /* evaluate the action
							matrix due to inertia
							and weight (12) */
			sum4(FI[i],ACT0[i+1],ACT0[i]);  /* evaluate total
							action matrix (13) */
		}
					      /* ***** OUTPUT RESULTS ***** */
		for(i=1;i<=nlink;i++)         /* for each link */
